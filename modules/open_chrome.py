...
#     else: driver = webdriver.Chrome(options=options, version_main=145) #, service=Service(executable_path="C:\\Program Files\\Google\\Chrome\\chromedriver-win64\\chromedriver.exe"))
...
def createChromeSession(isRetry: bool = False):
    ...
    # Stability flags for Chrome 120+ on macOS
    ...
    if stealth_mode:
        print_lg("Downloading Chrome Driver... This may take some time. Undetected mode requires download every run!")
        # use_subprocess=True fixes "Browser window not found" on macOS with Chrome 120+
        driver = uc.Chrome(options=options, version_main=145, use_subprocess=True)
    else:
        # version_main is a uc-only param; do NOT pass it to standard webdriver.Chrome
    ...
    try:
        ...
        pass  # Window maximize failed but driver is still usable
    wait = WebDriverWait(driver, 5)
    actions = ActionChains(driver)
    return options, driver, actions, wait
